...
```{r}
KingHouses <- read.csv('kc_house_data.csv', header = T, na.strings = '?')
KingHouses2 <- readr::read_csv('kc_house_data.csv')
...
KingHousesMin <- KingHouses[,c(3,6)]
...
```{r}
cor(KingHousesMin, method = 'pearson')
```

```{r}
cor(KingHousesMin, method = 'spearman')
...
```{r}
plot(y = log10(KingHousesMin$price),x = log10(KingHousesMin$sqft_living), main = 'Relación entre Precio y Area', ylab = 'precio', xlab = 'Area (sqft)')
...
hist(KingHousesMin$price)

preciolog10 = log10(KingHousesMin$price)
preciolog = log(KingHousesMin$price)
