...
utilizando como variable objetivo mpg y como variables predictoras hp, disp y weight, se realiza un histograma para observar el comportamiento de las variables
...
luego se crean hitogramas de las mismas variables pero esta vez en escala logaritmica
...
```{r}
line_model <- lm(mpg ~ hp + weight + disp, 
                 data = training_data)

predicted_data  <- predict(line_model, 
                           test_data)
predicts_real <- data.frame(cbind(real=test_data$mpg, 
...
```{r}
plot(autos$disp, autos$mpg)
plot(log(autos$disp), autos$mpg)
plot(autos$disp, log(autos$mpg))
plot(log(autos$disp), log(autos$mpg))

plot(sqrt(autos$disp), sqrt(autos$mpg))
