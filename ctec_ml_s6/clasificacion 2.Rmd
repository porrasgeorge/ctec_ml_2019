...
t1 <- Sys.time()
cosevi_training2 <- cosevi_training[1:1000,]
...
predicciones.red <- neuralnet::compute(cosevi_nn_model,cosevi_modMatrix_Test)
predicciones.redClass = ifelse(predicciones.red$net.result >= 0.5, T, F)

table(cosevi_test$HayHerido, predicciones.redClass)
...
prediccionesSVM = ROCR::prediction(c(svm_prediccion), 
                             c(cosevi_test$HayHerido))
                             
as.numeric(performance(prediccionesSVM, "auc")@y.values)

```

```{r}
sum(duplicated(cosevi))
sum(!duplicated(cosevi))
...
```{r}
cosevi %>% group_by(HayHerido, Rol, Edad_quinquenal, Sexo, Dia) %>% 
  dplyr::summarise(count(HayHerido), 
            first(HayHerido), 
            first(Rol), 
            first(Edad_quinquenal), 
            first(Sexo), 
            first(Dia))
```
