...
cosevi <- datos %>% 
  mutate(HayHerido = as.logical(ifelse(!Tipo_Lesion %in% c("Ileso"), T, F))) %>%
...
prediccionesROC = prediction(c(predicted_Arbol_modelo), 
...
prediccionesRandomForest = prediction(c(predicciones.bosque), 
                             c(cosevi_test$HayHerido))
...
predicciones_LR_ROCR = prediction(c(predicciones_LR), 
                             c(cosevi_test$HayHerido))
```
